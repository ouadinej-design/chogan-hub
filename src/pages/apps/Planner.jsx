...
// Calendrier stratégique 2026
const STRATEGIC = [
  { n:'Aïd el-Fitr',      d:'2026-03-20', t:'mus'    },
  { n:'Aïd el-Adha',      d:'2026-05-27', t:'mus'    },
  { n:'Mawlid',           d:'2026-09-10', t:'mus'    },
  { n:'Noël',             d:'2026-12-25', t:'cath'   },
  { n:'Saint-Valentin',   d:'2026-02-14', t:'comm'   },
  { n:'Fête des Mères',   d:'2026-05-31', t:'comm'   },
  { n:'Fête des Pères',   d:'2026-06-21', t:'comm'   },
  { n:'Black Friday',     d:'2026-11-27', t:'comm'   },
  { n:'Printemps',        d:'2026-03-20', t:'saison' },
  { n:'Été',              d:'2026-06-21', t:'saison' },
  { n:'Automne',          d:'2026-09-23', t:'saison' },
  { n:'Hiver',            d:'2026-12-21', t:'saison' },
];
const TYPE_COLOR = { mus:'#059669', cath:'#7c3aed', comm:'#d97706', saison:'#0891b2' };
const TYPE_LABEL = { mus:'Musulman', cath:'Chrétien', comm:'Commerce', saison:'Saison' };

export default function Planner() {
  const [tab, setTab] = useState('agenda');
  return (
    <AppLayout title="Planner" icon="🗓">
...
function EvenementsTab() {
  ...
  const [filter, setFilter] = useState('perso'); // 'perso' | 'strategique'
  const [success, setSuccess] = useState('');

  const now = useMemo(() => { const d=new Date(); d.setHours(0,0,0,0); return d; }, []);
  const upd = k => e => setForm(p => ({ ...p, [k]:e.target.value }));

  const saveEvts = (e) => { setEvts(e); localStorage.setItem('le_cevents', JSON.stringify(e)); };
  ...
  const save = () => {
    ...
    let e = [...evts];
    if (editId) {
      e = e.map(x => x.id===editId ? {...x, n:form.name.trim(), lbl:form.type, d:form.date, email:form.email, tel:form.tel} : x);
      setSuccess('✦ Événement modifié !');
    } else {
      e = [...e, { id:Date.now().toString(), n:form.name.trim(), lbl:form.type, d:form.date, email:form.email.trim(), tel:form.tel.trim() }];
      setSuccess('✦ Événement ajouté !');
    }
    saveEvts(e);
    setEditId(null);
    setForm({ name:'', type:'Salon/Expo', date:new Date().toISOString().split('T')[0], email:'', tel:'' });
    setTimeout(() => setSuccess(''), 2500);
  };

  const del = (id) => { if (!window.confirm('Supprimer ?')) return; saveEvts(evts.filter(x=>x.id!==id)); };
  ...
  const diff = d => Math.ceil((new Date(d)-now)/86400000);

  const DiffBadge = ({ d }) => {
    const days = diff(d);
    const color = days < 0 ? 'var(--text-dim)' : days <= 7 ? 'var(--red)' : days <= 30 ? '#d97706' : 'var(--green)';
    const label = days < 0 ? `J+${Math.abs(days)}` : days === 0 ? "Auj." : `J-${days}`;
    return <span style={{ fontSize:10, fontWeight:700, color, background:`${color}12`, padding:'2px 8px', borderRadius:20, border:`1px solid ${color}30`, flexShrink:0 }}>{label}</span>;
  };
  ...
  return (
    <div style={S.pad}>
      ...
      <div style={S.section}>
        <p style={S.secLabel}>{editId ? '✏️ Modifier l\'événement' : '➕ Ajouter un événement'}</p>
        {success && <div style={S.ok}>{success}</div>}
        <div className="field"><label className="label">Titre *</label>
          <input placeholder="Ex: Salon de Beauté..." value={form.name} onChange={upd('name')} /></div>
        ...
        <div className="field"><label className="label">Date *</label>
          <input type="date" value={form.date} onChange={upd('date')} /></div>
        <div className="grid-2">
          <div className="field"><label className="label">E-mail contact</label>
            <input type="email" placeholder="contact@email.com" value={form.email} onChange={upd('email')} /></div>
          <div className="field"><label className="label">Téléphone</label>
            <input type="tel" placeholder="+33 6..." value={form.tel} onChange={upd('tel')} /></div>
        </div>
      ...
      {/* Toggle perso/stratégique */}
      <div style={{ display:'flex', gap:8, marginBottom:14 }}>
        {[['perso','📅 Mes événements'],['strategique','🗓 Calendrier 2026']].map(([k,l]) => (
      ...
      {/* Mes événements perso */}
      {filter === 'perso' && (
        <>
          {futurs.length > 0 && (
            <>
              <p style={{ fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', color:'var(--or-deep)', marginBottom:8 }}>✦ Prochains — {futurs.length}</p>
              {futurs.map(ev => (
                <div key={ev.id} style={{ ...S.evCard, borderLeft:'3px solid var(--blue)' }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <p style={{ fontSize:12, fontWeight:600, color:'var(--taupe)' }}>{ev.n}</p>
                    <p style={{ fontSize:10, color:'var(--text-muted)', marginTop:2 }}>
                      {fmt(ev.d)} · <span style={{ color:'var(--blue)' }}>{ev.lbl}</span>
                    </p>
                    {ev.email && <p style={{ fontSize:9, color:'var(--text-dim)' }}>✉ {ev.email}</p>}
                    {ev.tel   && <p style={{ fontSize:9, color:'var(--text-dim)' }}>📞 {ev.tel}</p>}
                  </div>
                  <DiffBadge d={ev.d}/>
                  <div style={{ display:'flex', gap:4, marginLeft:6 }}>
                    <button style={S.iconBtn} onClick={() => openEdit(ev)}>✏️</button>
                    <button style={{ ...S.iconBtn, color:'var(--red)' }} onClick={() => del(ev.id)}>🗑</button>
                  </div>
                </div>
              ))}
            </>
          ...
          {passes.length > 0 && (
            <>
              <p style={{ fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', color:'var(--text-muted)', margin:'12px 0 8px' }}>📁 Historique — {passes.length}</p>
              {passes.map(ev => (
                <div key={ev.id} style={{ ...S.evCard, opacity:0.6 }}>
                  <div style={{ flex:1 }}>
                    <p style={{ fontSize:12, fontWeight:500, color:'var(--taupe)', textDecoration:'line-through' }}>{ev.n}</p>
                    <p style={{ fontSize:10, color:'var(--text-muted)' }}>{fmt(ev.d)} · {ev.lbl}</p>
                  </div>
                  <DiffBadge d={ev.d}/>
                  <button style={{ ...S.iconBtn, color:'var(--red)', marginLeft:6 }} onClick={() => del(ev.id)}>🗑</button>
                </div>
              ))}
            </>
          )}
          {!evts.length && <div style={S.empty}>Aucun événement. Ajoutez votre premier rendez-vous ci-dessus.</div>}
        </>
      )}
          ...
          {Object.entries(TYPE_COLOR).map(([type, color]) => {
            ...
            return (
              <div key={type} style={{ marginBottom:14 }}>
                <p style={{ fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', color, marginBottom:6 }}>{TYPE_LABEL[type]}</p>
                {items.map((e,i) => {
                  const days = diff(e.d);
                  const isPast = days < 0;
                  return (
                    <div key={i} style={{ ...S.evCard, borderLeft:`3px solid ${color}`, opacity:isPast?0.5:1 }}>
...
function VentesTab() {
  ...
  const del     = (id) => { if (!window.confirm('Supprimer cette vente ?')) return; save(sales.filter(s=>s.id!==id)); };

  const openEdit = (v) => {
    setEditing(v.id);
    setForm({ client:v.client||'', email:v.email||'', tel:v.tel||'', product:v.product||v.prod||'', qty:String(v.qty||1), category:v.category||v.cat||'Parfum', amount:String(v.amount||v.amt||''), currency:v.currency||v.cur||'€', date:v.date||new Date().toISOString().split('T')[0], note:v.note||'', consultant:v.consultant||'' });
  };
  const saveEdit = () => {
    save(sales.map(s => s.id !== editing ? s : { ...s, client:form.client, email:form.email, tel:form.tel, product:form.product, prod:form.product, qty:parseInt(form.qty)||1, category:form.category, cat:form.category, amount:parseFloat(form.amount)||0, amt:form.amount, currency:form.currency, cur:form.currency, date:form.date, note:form.note, consultant:form.consultant }));
    setEditing(null);
  };
  ...
  if (editing) return (
    <div style={S.pad}>
      ...
      <div style={S.section}>
        <p style={S.secLabel}>✏️ Modifier la vente</p>
    ...
    <div style={S.pad}>
      <div style={{ display:'flex', alignItems:'center', background:'var(--bg-card)', border:'1px solid var(--or-border)', borderRadius:14, padding:'12px 16px', marginBottom:14, gap:8 }}>
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', flex:1 }}>
          <span style={{ fontFamily:'var(--font-display)', fontSize:20, color:'var(--or-deep)', lineHeight:1 }}>{sales.length}</span>
          <span style={{ fontSize:9, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.06em' }}>Ventes</span>
        </div>
        <div style={{ width:1, height:30, background:'var(--or-border)' }}/>
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', flex:1 }}>
          <span style={{ fontFamily:'var(--font-display)', fontSize:17, color:'var(--or-deep)', lineHeight:1 }}>{totalEur.toFixed(2)}€</span>
          <span style={{ fontSize:9, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.06em' }}>CA €</span>
        </div>
        {totalDa > 0 && <>
          <div style={{ width:1, height:30, background:'var(--or-border)' }}/>
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', flex:1 }}>
            <span style={{ fontFamily:'var(--font-display)', fontSize:14, color:'var(--or-deep)', lineHeight:1 }}>{totalDa.toLocaleString('fr-FR')}</span>
            <span style={{ fontSize:9, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.06em' }}>CA DA</span>
          </div>
        </>}
        <button style={{ background:'var(--or-pale)', border:'1px solid var(--or-border)', color:'var(--or-deep)', borderRadius:8, padding:'6px 10px', cursor:'pointer', fontSize:14, fontFamily:'var(--font-body)' }} onClick={refresh}>↺</button>
      </div>

      <input placeholder="🔍 Cliente ou produit..." value={search} onChange={e=>setSearch(e.target.value)} style={{ marginBottom:12 }} />

      {filtered.length === 0
        ? <div style={S.empty}>Aucune vente. Enregistrez une vente depuis l'onglet Agenda ou l'app Commandes.</div>
        : filtered.map(v => {
            ...
            <div key={v.id} style={S.venteCard}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:6 }}>
                ...
                <div style={{ textAlign:'right', flexShrink:0 }}>
                  ...
                  <p style={{ fontSize:10, color:'var(--text-muted)', marginTop:2 }}>{fmt(v.date||v.createdAt)}</p>
                </div>
              </div>
              {items.length > 0
                ? <div style={{ background:'rgba(210,183,149,0.06)', borderRadius:8, padding:'8px 10px', marginBottom:8 }}>
                    {items.map((it,i) => (
                      <div key={i} style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 0', borderBottom:'1px solid rgba(210,183,149,0.12)' }}>
                        ...
                        <span style={{ fontSize:11, fontWeight:700, color:'var(--or-deep)', marginLeft:4 }}>{it.amt} {currency}</span>
                      </div>
                    ))}
                  </div>
                : product ? <p style={{ fontSize:11, color:'var(--text-muted)', marginBottom:8, lineHeight:1.5 }}>{product}</p> : null
              }
              {v.note && <p style={{ fontSize:11, color:'var(--text-muted)', marginBottom:8, fontStyle:'italic' }}>📝 {v.note}</p>}
              {v.consultant && <p style={{ fontSize:10, color:'var(--text-dim)', marginBottom:8 }}>Consultante : {v.consultant}</p>}
...
const S = {
  ...
  section:   { background:'var(--bg-card)', border:'1px solid var(--or-border)', borderRadius:14, padding:14, marginBottom:14 },
  secLabel:  { fontSize:11, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'var(--or-deep)', marginBottom:10 },
  ok:        { background:'rgba(74,124,89,0.1)', border:'1px solid rgba(74,124,89,0.3)', borderRadius:10, padding:'8px 12px', color:'var(--green)', fontSize:12, fontWeight:600, marginBottom:10 },
  evCard:    { display:'flex', alignItems:'center', gap:10, background:'var(--bg-card)', border:'1px solid var(--or-border)', borderRadius:12, padding:'10px 12px', marginBottom:8 },
  venteCard: { background:'var(--bg-card)', border:'1px solid var(--or-border)', borderRadius:14, padding:'12px 14px', marginBottom:10 },
  iconBtn:   { background:'none', border:'none', cursor:'pointer', fontSize:14, padding:'2px 5px', color:'var(--or-deep)', fontFamily:'var(--font-body)' },
