...
export default function Reseau() {
  ...
  const [tree, setTree] = useState(() => {
    try {
      ...
      return localData ? JSON.parse(localData) : { nodes: initialNodes };
    } catch (e) { return { nodes: initialNodes }; }
  });

  const [newMember, setNewMember] = useState({ name: '', role: 'Consultante', genre: 'femme', parentId: '', mdp: '', caNum: '0', objNum: '1000' });
  ...
  const handleAddMember = () => {
    ...
    const newNode = { ...newMember, id: Date.now().toString(), ca: `${newMember.caNum} €`, objectif: "Non défini", events: "Aucun", fidelity: "Initial" };
    setTree({ nodes: [...tree.nodes, newNode] });
    setNewMember({ name: '', role: 'Consultante', genre: 'femme', parentId: '', mdp: '', caNum: '0', objNum: '1000' });
  };
  ...
  return (
    <div style={{ padding: '20px', background: '#FAF7F2', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        <button onClick={() => setActiveTab('arbre')} style={{ padding: '8px 16px', borderRadius: '20px', border: '1px solid #D2B795', background: activeTab === 'arbre' ? '#D2B795' : 'white' }}>Arbre</button>
        <button onClick={() => setActiveTab('tableau')} style={{ padding: '8px 16px', borderRadius: '20px', border: '1px solid #D2B795', background: activeTab === 'tableau' ? '#D2B795' : 'white' }}>Gestion</button>
      </div>
      ...
      {selectedMember && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', padding: '20px', borderRadius: '16px', width: '300px' }}>
            ...
            <p><strong>Best Seller :</strong> {selectedMember.bestSeller}</p>
