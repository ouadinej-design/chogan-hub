...
const Gauge = ({ percentage }) => {
  ...
  return (
    <svg width="80" height="80" viewBox="0 0 80 80">
      ...
      <circle cx="40" cy="40" r={radius} fill="none" stroke="#2d7a4a" strokeWidth="8" 
              strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} 
              strokeLinecap="round" transform="rotate(-90 40 40)" />
      <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle" 
            fontSize="16" fontWeight="bold" fill="#4A3E3D">{percentage}%</text>
    </svg>
  );
};
...
const PerformanceBadge = ({ ca, obj }) => {
  ...
  if (pct >= 100) return (
    <span style={{ background: '#d4edda', color: '#155724', padding: '2px 8px', borderRadius: '10px', fontSize: '9px', fontWeight: 'bold' }}>
      🔥 TOP
    </span>
  );
  if (pct >= 70) return (
    <span style={{ background: '#fff3cd', color: '#856404', padding: '2px 8px', borderRadius: '10px', fontSize: '9px', fontWeight: 'bold' }}>
      ⚡ OK
    </span>
  );
  return (
    <span style={{ background: '#f8d7da', color: '#721c24', padding: '2px 8px', borderRadius: '10px', fontSize: '9px', fontWeight: 'bold' }}>
      ⚠️ RELANCER
    </span>
  );
};
...
export default function Reseau() {
  ...
  const [newMember, setNewMember] = useState({ 
    ...
    caNum: '0', objNum: '1000', objectif: '', topClient: '', bestSeller: '', ventes: '' 
  });
    ...
    return (
      <div key={node.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div onClick={() => setSelectedMember(node)} 
             style={{ background: 'white', border: '1px solid #D2B795', borderRadius: '12px', padding: '10px', minWidth: '140px', textAlign: 'center', cursor: 'pointer', margin: '5px' }}>
        ...
        {enfants.length > 0 && (
          <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
            {enfants.map(e => renderTreeNodes(e))}
          </div>
        )}
      </div>
    );
      ...
      {selectedMember && (
        <div style={{ position: 'fixed', top:0, left:0, width:'100%', height:'100%', background: 'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex: 1000 }}>
          <div style={{ background: 'white', padding: '20px', borderRadius: '16px', width: '320px' }}>
            ...
            <button onClick={() => setSelectedMember(null)} style={{ marginTop: '10px', padding: '5px 15px' }}>Fermer</button>
          </div>
        </div>
      )}
    </div>
